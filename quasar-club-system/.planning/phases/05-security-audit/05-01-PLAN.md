...
**writeAuditLog(entry: Omit<IAuditLog, 'id'>): Promise<void>** — Fire-and-forget pattern. Callers should NOT await this function. Internally:
...
- Returns Promise<void> from the chain. Add JSDoc: `/** Fire-and-forget audit log write. Do NOT await — audit failures must not block operations. */`
...
IMPORTANT: `writeAuditLog` returns `Promise<void>` — callers must NOT await it (fire-and-forget). The JSDoc comment documents this intent clearly.
  </action>
  <verify>
