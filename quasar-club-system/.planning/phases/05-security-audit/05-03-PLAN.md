...
<task type="auto">
  <name>Task 2: Add audit logging to cashbox operations and update all callers</name>
  <files>src/services/cashboxFirebase.ts, src/composable/useSurveyUseCases.ts, src/composable/useCashboxUseCases.ts, src/pages/SingleTeamPage.vue</files>
  <action>
...
**IMPORTANT — `removeMember` is called directly from `src/pages/SingleTeamPage.vue` (no use case composable layer).** Update SingleTeamPage.vue to pass audit context when calling `removeMember`:
- Find the call to `teamFirebase.removeMember(teamId, memberUid)` (or similar)
- Update to: `teamFirebase.removeMember(teamId, memberUid, { actorUid: authStore.user!.uid, actorDisplayName: authStore.user!.displayName || authStore.user!.email || 'Unknown', memberDisplayName: member.displayName })`
- Import `useAuthStore` if not already imported

For any other operations where composable callers don't exist, search for all callers using grep for the function names and update at the page/component level.
