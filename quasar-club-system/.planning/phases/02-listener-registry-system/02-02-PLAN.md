...
<task type="auto">
  <name>Task 2: Migrate component-level listeners and add team switching cleanup</name>
...
**In src/pages/NotificationsPage.vue:**
Import `listenerRegistry` from `@/services/listenerRegistry`.
- Replace local `let unsubscribe = null` with registry registration.
- Where notification listener is created, register: `listenerRegistry.register('notifications', unsubscribe, { userId })`.
- Where cleanup happens, replace with `listenerRegistry.unregister('notifications')`.
- Remove the local `unsubscribe` variable.

**In src/components/notifications/NotificationsDropdown.vue:**
Import `listenerRegistry` from `@/services/listenerRegistry`.
- Same pattern as NotificationsPage: register with `listenerRegistry.register('notifications', ...)` and clean up with `listenerRegistry.unregister('notifications')`.
...
NOTE: NotificationsDropdown and NotificationsPage both use the 'notifications' ID. Since only one can be active at a time (dropdown is in layout, page replaces content), the auto-cleanup on re-register handles this correctly.
  ...
  <verify>Run `quasar build` to confirm no compilation errors. Grep for `let unsubscribe = null` in the modified component files to confirm local unsubscribe variables are removed. Grep for `listenerRegistry` in each modified file to confirm registration.</verify>
  <done>All 9 listener types (auth, teams, surveys, notifications, messages, cashbox-fines/payments/rules/history) registered with ListenerRegistry. No orphan unsubscribe storage. Team switching cleans up team-scoped listeners. Build passes.</done>
</task>
