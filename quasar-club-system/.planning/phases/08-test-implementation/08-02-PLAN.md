...
must_haves:
  truths:
    ...
    - "Auth initializeAuth test sets authReady and registers auth listener"
...
- failure (AuthError): loginUser rejects with AuthError('auth/wrong-password') → authStore.user is null, notifyError called with error message, loading goes false, error re-thrown
- failure (network error): loginUser rejects with AuthError('auth/network-request-failed') → notifyError called with retry option
...
**initializeAuth:**
- with null user: authStateReady resolves null → authStore.authReady set true, no team listener setup
...
- Router navigation verified on logout (push to HOME)
</success_criteria>
