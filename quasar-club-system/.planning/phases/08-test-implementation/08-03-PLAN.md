...
- **Firebase error (FirestoreError)**: Make `mockAddOrUpdateVote.mockRejectedValueOnce(new FirestoreError('unavailable', 'Service unavailable'))`. Pre-populate surveys. Call `voteOnSurvey(...)`. Verify `notifyError` called with error message and retry option. Verify error re-thrown.
...
- **Transient error has retry**: Make mock reject with FirestoreError code 'unavailable'. Verify notifyError called with `{ retry: true, onRetry: expect.any(Function) }`.

- **Permanent error no retry**: Make mock reject with FirestoreError code 'permission-denied'. Verify notifyError called WITHOUT retry option (shouldRetry is false because 'permission-denied' is not 'unavailable' or 'deadline-exceeded').
